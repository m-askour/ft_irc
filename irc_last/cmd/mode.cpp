...
void server::mode(client *cl, const std::vector<std::string> &args)
{
    ...
    for (size_t i = 0; i < modeString.size(); i++)
    {
		...
		case 'o':
		{
			...
			if (!target)
			{
				sendReply(cl, "401", nickname, "No such nick/channel");
				return;
			}

			if (channel->users.find(target) == channel->users.end())
			{
				sendReply(cl, "44545", nickname, "They aren't on that channel");
				return;
			}
    ...
    std::string broadcast = ":" + prefix + " MODE " + channelName + " " + appliedModes + appliedParams + "\n";
